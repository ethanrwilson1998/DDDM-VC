...
def compute_pitch_variability(audio_path, sr=16000, fmin='C2', fmax='C7'):
    ...
    if len(vf0)<2:
        ...
        slopes = np.diff(vf0)/np.diff(vt)
    return {
        'mean_pitch': mean_p,
        'std_pitch': std_p,
        'pitch_range': prange,
        'cv': cv,
        'f0': f0, 'times': times, 'slopes': slopes
    }
    ...
    for wav in sorted(glob.glob(os.path.join(root_folder,'**','*.wav'),recursive=True)):
        try:
            ...
            row={
                ...
                'pitch_range':m['pitch_range'],'cv':m['cv'],
                ...
                'cv_slope':   np.nan if len(m['slopes'])<1 else (m['slopes'].std()/m['slopes'].mean() if m['slopes'].mean() else np.nan),
                'f0':    json.dumps(m['f0'].tolist()),
                'times': json.dumps(m['times'].tolist()),
                'slopes':json.dumps(m['slopes'].tolist())
            }
            rows.append(row)
            # print(f"{wav}: mean={m['mean_pitch']:.2f}, cv={m['cv']:.2f}")
            # print(f"f0 = {m['f0']}, times = {m['times']}, slopes = {m['slopes']}")
